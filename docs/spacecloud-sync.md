...
- After a direct-added upload, the watcher verifies the SpaceCloud calendar with staged page reloads instead of polling one stale DOM for 90 seconds. A candidate is accepted only after its popup matches the task room, date/time, reserver identity, and Naver reservation number; this handles delayed SpaceCloud visibility without weakening duplicate protection.
...
- If `RHYTHMJOY_SPACECLOUD_NAVER_BLOCK_ENABLED=1` is also enabled, SpaceCloud reservation-complete emails create a `naver_block` task. The active browser watcher opens Naver SmartPlace, selects the mapped room/date/time, splits overnight or multi-hour reservations into hourly slots, and changes available slots to `예약불가`. If Naver already has a confirmed or sold-out slot there, the watcher does not overwrite it and marks the task `needs_review`.
