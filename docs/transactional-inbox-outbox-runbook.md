...
Google Calendar 조회, Telegram 알림 등 외부 네트워크 호출은 DB 트랜잭션 밖에서 실행한다. 트랜잭션 안에서 외부 호출을 기다리면 잠금 장기화와 불확실한 재시도가 생긴다.
...
1. 원본 플랫폼과 이메일, Inbox, 논리 원장, Outbox, Google Calendar 순으로 사실을 확인한다.
