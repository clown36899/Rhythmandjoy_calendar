// 💡 [수정] window.CONFIG를 덮어쓰지 않고, 기존 객체에 속성을 병합합니다.
// 이렇게 하면 env.js에서 설정한 API 키가 유지됩니다.
...
// 💡 [핵심 수정] env.js에서 로드된 전역 API 키를 CONFIG 객체 안으로 복사합니다.
// 이 로직의 누락이 이전 모든 '설정 파일 로드' 오류의 근본 원인이었습니다.
if (window.GOOGLE_CALENDAR_API_KEY) {
  window.CONFIG.googleCalendarApiKey = window.GOOGLE_CALENDAR_API_KEY;
}
...
// 🔴 [수정] console.log, info, warn을 덮어쓰는 코드를 제거합니다.
// 이 코드는 Supabase Realtime 라이브러리의 정상적인 동작을 방해하여 메시지 수신을 막는 원인이었습니다.
