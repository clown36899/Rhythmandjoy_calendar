...
var swiper_esm_bundle_defaults = {
  init: true,
...
class swiper_esm_bundle_Swiper extends SwiperClass {
  constructor(...args) {
    ...
    const swiperParams = Utils.extend({}, swiper_esm_bundle_defaults);
  ...
  static get defaults() {
    return swiper_esm_bundle_defaults;
  }
...
class swipeCalendar_SwipeCalendar {
  constructor(element, options) {
    const swiperInit = swiper_esm_bundle.prototype.init;

    swiper_esm_bundle.prototype.init = function () {
    ...
    this._viewSkeletonDestroyCb = this._options.viewSkeletonDestroy;
    this._datesRenderCb = this._options.datesRender;
    this._datesDestroyCb = this._options.datesDestroy;
  ...
  _init() {
    ...
    if (this._options.themeSystem === 'bootstrap') {
      ...
      this._element.classList.remove('fc-unthemed');
    } else {
      this._element.classList.add('fc-unthemed');

      this._element.classList.remove('fc-bootstrap');
    } // Calendar Header
    ...
    if (!this._isLicenseKeyValid(this._options.swipeLicenseKey)) {
      this._noLicenseKey = true;

      this._showTrialMessage();

      this._element.appendChild(this._getCreditLink());
   ...
   * @see {@link https://fullcalendar.io/docs/Calendar-rerenderEvents} for further information.
   */


  rerenderEvents() {
    this._applyMethod('rerenderEvents');
   ...
   * @see {@link https://fullcalendar.io/docs/rerenderResources} for further information.
   */


  rerenderResources() {
    this._applyMethod('rerenderResources');
  } // Private methods
  ...
  _cleanUp() {
    ...
    if (this._curCal) {
      if (this._datesDestroyCb) {
        this._datesDestroyCb({
          view: this._curCal.view,
          el: this._curCal.el.querySelector('.fc-view')
        });
      }

      if (this._viewSkeletonDestroyCb) {
        this._viewSkeletonDestroyCb({
          view: this._curCal.view,
          el: this._curCal.el.querySelector('.fc-view')
        });
      }
  ...
  _isLicenseKeyValid(key) {
    ...
    if (utils_hash(key.replace(/-/g, '')) !== 12824938) {
      return false;
    }
  ...
  _getCurrentViewAmount() {
    const dateIncrement = this._curCal.state.dateProfile.dateIncrement;
    let amount = 0;
  ...
  _initCalendar(dateId, date) {
    ...
    if (this._options.swipeTitlePosition === 'left') {
      headerTitleSettings = {
        left: 'title',
        center: '',
        right: ''
      };
    } else if (this._options.swipeTitlePosition === 'center') {
      headerTitleSettings = {
        left: '',
        center: 'title',
        right: ''
      };
    } else if (this._options.swipeTitlePosition === 'right') {
      headerTitleSettings = {
        left: '',
        center: '',
        right: 'title'
    ...
    let height = this._calendarHeight;

    if (this._calendarHeight === 'parent' || !isNaN(parseFloat(this._calendarHeight)) && isFinite(this._calendarHeight)) {
      height = () => {
        let visibleHeight = 0;
        ...
        if (this._calendarHeight === 'parent') {
          ...
          visibleHeight = Math.floor(parentElemHeight - (headerHeight + footerHeight + creditHeight));
        } else {
    ...
    const calOptions = mergeDeep(this._options, {
      header: headerTitleSettings != null ? headerTitleSettings : false,
      footer: false,
      height: height,
      ...
      navLinkDayClick: (date, jsEvent) => {
        ...
        if (this._options.navLinks) {
          this._currentView = 'dayGridDay';
          this._currentDate = moment_default()(date);

          this._initSwiper();

          if (this._navLinkDayClickCb) {
  ...
  _initSwiper() {
    ...
    this.swiperConfig = {
      direction: 'horizontal',
      effect: this._options.swipeEffect,
      speed: this._options.swipeSpeed,
      noSwipingClass: 'fc-draggable',
      simulateTouch: true,
      roundLengths: true,
      slidesPerView: 1,
      preventClicks: false,
    ...
    this._swiper.prependSlide(this._getCalendarSlide(previousDateId));

    this._swiper.appendSlide(this._getCalendarSlide(nextDateId));
    ...
    if (this._viewSkeletonRenderCb) {
      this._viewSkeletonRenderCb({
        view: this._curCal.view,
        el: this._curCal.el.querySelector('.fc-view')
      });
    }

    if (this._datesRenderCb) {
      this._datesRenderCb({
        view: this._curCal.view,
        el: this._curCal.el.querySelector('.fc-view')
      });
  ...
  _updateToolbars() {
    ...
    if (this._todayButtonElement) {
      const today = moment_default()();
      const currentRangeUnit = this._curCal.state.dateProfile.currentRangeUnit;

      if (this._currentDate.isSame(today, currentRangeUnit)) {
        this._todayButtonElement.disabled = true;

        this._todayButtonElement.classList.add('fc-state-disabled');
      } else {
        this._todayButtonElement.disabled = false;

        this._todayButtonElement.classList.remove('fc-state-disabled');
      }
    }
    ...
    for (let i = 0; i < activeButtonElmts.length; i++) {
      activeButtonElmts[i].classList.remove('active', 'fc-state-active', 'ui-state-active');
    }
  ...
  _getButton(action, cornerLeft, cornerRight) {
    ...
    if (action === 'title') {
      ...
      element.className = 'fc-toolbar-view-title';
      ...
      if (this._options.themeSystem === 'bootstrap') {
        ...
        if (cornerLeft !== false) {
          element.classList.add('fc-corner-left');
        }

        if (cornerRight !== false) {
          element.classList.add('fc-corner-right');
        }
      ...
      if (action === this._currentView) {
        element.classList.add('fc-state-active', 'active', 'ui-state-active');
      }
  ...
  _getToolbarPositionActions(toolbarPositionActions, position) {
    const element = document.createElement('div');
  ...
  _getToolbar(toolbarDef, isHeader) {
    ...
    if (toolbarDef) {
      ...
      if (isHeader) {
        element.className = 'fc-toolbar fc-header-toolbar swc-toolbar';
      } else {
        element.className = 'fc-toolbar fc-footer-toolbar swc-toolbar';
      }

      if (toolbarDef.left) {
        element.appendChild(this._getToolbarPositionActions(toolbarDef.left, 'left'));
      }

      if (toolbarDef.center) {
        element.appendChild(this._getToolbarPositionActions(toolbarDef.center, 'center'));
      }

      if (toolbarDef.right) {
        element.appendChild(this._getToolbarPositionActions(toolbarDef.right, 'right'));
      }
    }
