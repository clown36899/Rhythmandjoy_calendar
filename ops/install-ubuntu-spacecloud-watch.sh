...
NOTIFY_STATE="$LOG_DIR/notify-state.json"
...
LEGACY_NOTIFY_STATE="$REPO_ROOT/state/spacecloud-watch/notify-state.json"
if [[ ! -e "$NOTIFY_STATE" && -f "$LEGACY_NOTIFY_STATE" ]]; then
  install -m 600 "$LEGACY_NOTIFY_STATE" "$NOTIFY_STATE"
fi
...
ExecStart=$XVFB_RUN_BIN -a $NODE_BIN $REPO_ROOT/tools/spacecloud-watch.mjs watch --interval-seconds $INTERVAL_SECONDS --remote-outage-alert-seconds $REMOTE_OUTAGE_ALERT_SECONDS --limit-per-cycle $LIMIT_PER_CYCLE --delete-limit-per-cycle $DELETE_LIMIT_PER_CYCLE --naver-block-limit-per-cycle $NAVER_BLOCK_LIMIT_PER_CYCLE --naver-cancel-limit-per-cycle $NAVER_CANCEL_LIMIT_PER_CYCLE --spacecloud-cancel-limit-per-cycle $SPACECLOUD_CANCEL_LIMIT_PER_CYCLE $EXTRA_ARGS --daily-reconcile-hour $DAILY_RECONCILE_HOUR --daily-reconcile-state $LOG_DIR/daily-reconcile-state.json --notify-state $NOTIFY_STATE --admin-platform-audit-interval-minutes $ADMIN_PLATFORM_AUDIT_INTERVAL_MINUTES --admin-platform-audit-limit $ADMIN_PLATFORM_AUDIT_LIMIT --admin-platform-audit-state $LOG_DIR/admin-platform-audit-state.json --customer-platform-audit --customer-platform-audit-interval-minutes $CUSTOMER_PLATFORM_AUDIT_INTERVAL_MINUTES --customer-platform-audit-limit $CUSTOMER_PLATFORM_AUDIT_LIMIT --customer-platform-audit-state $LOG_DIR/customer-platform-audit-state.json --no-reflection-audit --env-file $ENV_FILE --profile-dir $PROFILE_DIR --work-dir $LOG_DIR
Restart=always
RestartSec=10
