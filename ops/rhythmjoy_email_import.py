...
def ensure_db_tables(config, logger):
    ...
    try:
        ...
        try:
            ...
            with migration_conn.cursor() as cursor:
                audit_trusted_naver_ledger_pointers(cursor)
                audit_naver_ledger_projection_invariants(cursor)
                reprojected_fenced_count = fence_reprojected_canceled_uploads(
                    cursor,
                    target_ledger_ids,
                )
                recovered_counts = recover_reprojected_skipped_uploads(
                    config,
                    cursor,
                    target_ledger_ids,
                )
                task_backfill_counts = backfill_safe_spacecloud_task_state(cursor)
                audit_naver_task_projection_invariants(cursor)
        ...
        if reprojected_naver_count:
            logger.info(
                'Deterministically reprojected trusted Naver booking events '
                'count=%s changed_ledgers=%s target_ledgers=%s',
                reprojected_naver_count,
                len(changed_ledger_ids),
                len(target_ledger_ids),
            )
        if any(recovered_counts.values()):
            logger.warning(
                'Recovered reprojected never-submitted uploads=%s sms=%s',
                recovered_counts.get('uploads', 0),
                recovered_counts.get('sms', 0),
            )
        if reprojected_fenced_count:
            logger.warning(
                'Fenced stale uploads after Naver generation reproject count=%s',
                reprojected_fenced_count,
            )
