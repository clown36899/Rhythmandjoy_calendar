...
`RHYTHMJOY_SPACECLOUD_EMAIL_ENABLED=1` enables SpaceCloud reservation-complete email intake from the Naver mail folder displayed as `스페이스클라우드`. The Naver email plus DB row is the source of truth; Google Calendar checks are only downstream verification. With `RHYTHMJOY_SPACECLOUD_NAVER_BLOCK_ENABLED=1`, parsed SpaceCloud confirmations create `naver_block` tasks that the local Mac watcher applies to Naver SmartPlace before writing Google Calendar. Set either flag back to `0` and restart `my_email_service.service` to disable the new intake or queue without affecting the existing Naver reservation/cancellation importer.
