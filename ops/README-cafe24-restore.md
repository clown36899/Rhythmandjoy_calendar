...
SpaceCloud emails are matched without Naver reservation numbers. Room, use date, and normalized reserver name are the required business identity; whitespace is removed and trailing `님` suffixes are stripped. Each new confirmation is a separate ledger generation keyed by the SpaceCloud internal id when present or by its durable email event id otherwise. A cancellation narrows candidates by the exact start/end time and may select only one active generation received strictly before the cancellation. Same-order collisions, multiple prior candidates, missing chronology, and historical email without an active ledger are retained as `needs_review` instead of changing Naver availability automatically. Historical slot-natural ledger keys remain read-compatible and are not mass-migrated by backfill.
...
The Ubuntu watcher can additionally enable `--customer-platform-audit`. It reuses the existing persistent Naver/SpaceCloud session and checks one customer reservation every four hours by default. The source platform is always checked; the opposite platform is checked only when that booking has a completed mirror-task record. This prevents source-only historical imports from being mislabeled as missing mirrors. Local macOS installation explicitly disables this audit so it cannot open a visible browser unexpectedly. A list-search miss is not a confirmed mismatch; only an identity-verified explicit canceled state is promoted immediately. An inconclusive first read is rechecked after three minutes; only a consecutive failure becomes an operator alert.
